...
function Add-NetmonCommands ([@string]$arrCommands)
{

    #run nmcap /displaynetwork, store in ResArray
    #for each $line in ResArray
        #if matches 'eterhnet' or 'wi-fi'
        #push command onto $arrCommands)
        $arrCommands += "cmd.exe /c nmcap /network (first char of ($line)) /capture /file $PSScriptRoot\filename.cap:50M"
...
$arrCommands = Add-NetmonCommands $arrCommands
